 ...
 */
@objc public enum PulleyPosition: Int, RawRepresentable {
...
open class PulleyViewController: UIViewController, PulleyDrawerViewControllerDelegate {
    ...
    override open func viewDidLayoutSubviews() {
        ...
        if supportedPositions.contains(.open)
        {
            ...
            drawerScrollView.frame = CGRect(
                ...
                height: self.view.bounds.height - topInset - safeAreaTopInset
            )
        }
        else
        {
    ...
    func getStopList() -> [CGFloat] {
        ...
        {
            collapsedHeight = drawerVCCompliant.collapsedDrawerHeight(bottomSafeArea: pulleySafeAreaInsets.bottom)
            partialRevealHeight = drawerVCCompliant.partialRevealDrawerHeight(bottomSafeArea: pulleySafeAreaInsets.bottom)
        }
    ...
    public func setDrawerPosition(position: PulleyPosition, animated: Bool, completion: PulleyAnimationCompletionBlock? = nil) {
        ...
        if animated && self.view.window != nil
        {
            ...
            UIView.animate(withDuration: animationDuration, delay: animationDelay, usingSpringWithDamping: animationSpringDamping, initialSpringVelocity: animationSpringInitialVelocity, options: animationOptions, animations: {
    ...
    public func setPrimaryContentViewController(controller: UIViewController, animated: Bool = true, completion: PulleyAnimationCompletionBlock? = nil)
    {
        ...
        if animated
        {
            UIView.transition(with: primaryContentContainer, duration: 0.5, options: .transitionCrossDissolve, animations: {
    ...
    public func setDrawerContentViewController(controller: UIViewController, animated: Bool = true, completion: PulleyAnimationCompletionBlock? = nil)
    {
        ...
        if animated
        {
            UIView.transition(with: drawerContentContainer, duration: 0.5, options: .transitionCrossDissolve, animations: {
    ...
    public func scrollViewDidEndDragging(_ scrollView: UIScrollView, willDecelerate decelerate: Bool) {
        ...
        case .nearestPositionUnlessExceeded(let threshold):
            ...
            if abs(distance) > threshold
            {
                if distance < 0
                {
                    let orderedSupportedDrawerPositions = supportedPositions.sorted(by: { $0.rawValue < $1.rawValue }).filter({ $0 != .hidden })
